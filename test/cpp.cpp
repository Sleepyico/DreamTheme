...
void InitMap(map<string, vector<string>> &svmap, ifstream &ifile) {
  ...
  while (getline(ifile, textline, '\n')) {
    ...
    if (!svmap.count(familyName))
      ...
      cerr << "Sorry, we already have a " << familyName
           << " family in our map!\n";
  }
}

void DisplayMap(map<string, vector<string>> &svmap, ofstream &ofile) {
  for (map<string, vector<string>>::iterator itr = svmap.begin(),
                                             mapEnd = svmap.end();
       itr != mapEnd; ++itr) {
    ...
    if (itr->second.empty())
      ...
      for (vector<string>::iterator itrvec = itr->second.begin(),
                                    vecEnd = itr->second.end();
...
void UserQuery(map<string, vector<string>> &svmap) {
  ...
  for (map<string, vector<string>>::iterator itr = svmap.begin(),
    ...
    if (itr->first == queryName) {
      cout << "The " << itr->first << " family has " << itr->second.size()
           << " children: ";
      for (vector<string>::iterator itrvec = itr->second.begin(),
                                    vecEnd = itr->second.end();
           itrvec != vecEnd; ++itrvec)
